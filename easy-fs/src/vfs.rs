...
impl Inode {
    ...
    pub fn create(&self, name: &str) -> Option<Arc<Inode>> {
        ...
        // return inode
        Some(
            Arc::new(Self::new(block_id, block_offset, self.fs.clone(), self.block_device.clone()))
        )
    ...
    /// get id of self without providing name, using fs
    pub fn get_inode_id(&self) -> usize {
        let fs = self.fs.lock();
        fs.get_inode_id(self.block_id, self.block_offset)
    }

    /// if the inode represent a dir
    pub fn is_dir(&self) -> bool {
        self.read_disk_inode(|disk_inode| disk_inode.is_dir())
    }

    /// if the inode represent a file
    ...
    pub fn get_nlink(&self, inode: &Arc<Inode>) -> u32 {
        ...
        // search from disk_inode as the root inode
        self.read_disk_inode(|disk_inode| {
            let file_num = (disk_inode.size as usize) / DIRENT_SZ;
            // write into this dir entry
            let mut dirent = DirEntry::empty();
            // for all file
            for i in 0..file_num {
                // read a dirent
                ...
                // if the dirent points to the param inode
                if (dirent.inode_number() as usize) == inode_id {
                    nlink += 1;
                }
            }
        });
